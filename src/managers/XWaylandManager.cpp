...
void CHyprXWaylandManager::getGeometryForWindow(PHLWINDOW pWindow, CBox* pbox) {
    if (pWindow->m_bIsX11) {
        const auto SIZEHINTS = pWindow->m_pXWaylandSurface->size_hints.get();
...
bool CHyprXWaylandManager::shouldBeFloated(PHLWINDOW pWindow, bool pending) {
    if (pWindow->m_bIsX11) {
        ...
        if (pWindow->m_pXWaylandSurface->role.starts_with("task_dialog") || pWindow->m_pXWaylandSurface->role.starts_with("pop-up"))
            return true;
        ...
        const auto SIZEHINTS = pWindow->m_pXWaylandSurface->size_hints.get();
        if (SIZEHINTS && (pWindow->m_pXWaylandSurface->parent || ((SIZEHINTS->min_width == SIZEHINTS->max_width) && (SIZEHINTS->min_height == SIZEHINTS->max_height))))
            return true;
    } else {
...
Vector2D CHyprXWaylandManager::getMaxSizeForWindow(PHLWINDOW pWindow) {
    ...
    if ((pWindow->m_bIsX11 && !pWindow->m_pXWaylandSurface->size_hints) || (!pWindow->m_bIsX11 && !pWindow->m_pXDGSurface->toplevel) || pWindow->m_sAdditionalConfigData.noMaxSize)
        return Vector2D(99999, 99999);

    auto MAXSIZE = pWindow->m_bIsX11 ? Vector2D(pWindow->m_pXWaylandSurface->size_hints->max_width, pWindow->m_pXWaylandSurface->size_hints->max_height) :
...
Vector2D CHyprXWaylandManager::getMinSizeForWindow(PHLWINDOW pWindow) {
    ...
    if ((pWindow->m_bIsX11 && !pWindow->m_pXWaylandSurface->size_hints) || (!pWindow->m_bIsX11 && !pWindow->m_pXDGSurface->toplevel))
        return Vector2D(0, 0);

    auto MINSIZE = pWindow->m_bIsX11 ? Vector2D(pWindow->m_pXWaylandSurface->size_hints->min_width, pWindow->m_pXWaylandSurface->size_hints->min_height) :
