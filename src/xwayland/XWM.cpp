...
void CXWM::handleMapRequest(xcb_map_request_event_t* e) {
    ...
    const bool SMALL =
        XSURF->geometry.size() < Vector2D{2, 2} || (XSURF->size_hints && XSURF->geometry.size() < Vector2D{XSURF->size_hints->min_width, XSURF->size_hints->min_height});
    const bool HAS_HINTS   = XSURF->size_hints && Vector2D{XSURF->size_hints->base_width, XSURF->size_hints->base_height} > Vector2D{5, 5};
    const auto DESIREDSIZE = HAS_HINTS ? Vector2D{XSURF->size_hints->base_width, XSURF->size_hints->base_height} : Vector2D{800, 800};
    ...
    if ((SMALL || HAS_HINTS) && !XSURF->overrideRedirect) // default to 800 x 800
        ...
        // otherwise, configure with its size
        auto cookie = xcb_get_geometry(connection, e->window);
        auto reply  = xcb_get_geometry_reply(connection, cookie, nullptr);
        if (reply)
            XSURF->configure({reply->x, reply->y, reply->width, reply->height});
        free(reply);
    }
...
static void readProp(SP<CXWaylandSurface> XSURF, uint32_t atom, xcb_get_property_reply_t* reply) {
    std::string propName = "?";
    ...
    if (atom == XCB_ATOM_WM_CLASS) {
        ...
        XSURF->state.appid = XSURF->state.appid.substr(0, XSURF->state.appid.find_first_of('\0')); // fuck you X
...
void CXWM::handleClientMessage(xcb_client_message_event_t* e) {
    ...
    } else if (e->type == HYPRATOMS["WM_STATE"]) {
        if (e->format == 32) {
...
void CXWM::readWindowData(SP<CXWaylandSurface> surf) {
    const std::array<xcb_atom_t, 5> interestingProps = {
        XCB_ATOM_WM_CLASS, XCB_ATOM_WM_NAME, HYPRATOMS["WM_HINTS"], HYPRATOMS["_NET_WM_STATE"], HYPRATOMS["_NET_WM_NAME"],
    };
...
void CXWM::associate(SP<CXWaylandSurface> surf, wlr_surface* wlSurf) {
    ...
    if (surf->windowCreated)
        Debug::log(WARN, "[xwm] Can't create window for association, window already exists");
    else {
        surf->windowCreated = true;

        const auto WINDOW = CWindow::create(surf);
        g_pCompositor->m_vWindows.emplace_back(WINDOW);
        WINDOW->m_pSelf = WINDOW;

        Debug::log(LOG, "New XWayland window at {:x} for surf {:x}", (uintptr_t)WINDOW.get(), (uintptr_t)surf.get());
    }
